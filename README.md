# humanoid_qinglong — 青龙

## 项目简介

青龙人形机器人应用包，31 自由度（左腿 6 + 右腿 6 + 腰 3 + 头 2 + 左臂 7 + 右臂 7）。包含该机型专属的 YAML 配置、MuJoCo 仿真资源、RL 策略模型及启动脚本，通用控制逻辑见 `humanoid_common` 仓库。
...
lunch k3-com260-kit-humanoid-qinglong
m
```
...
```bash
run_driver_qinglong.sh    # 终端1（PC，x86_64）
run_control_qinglong.sh   # 终端2（K3 板卡）
run_hmi_qinglong.sh       # 终端3（K3 板卡）
```

**sim2sim（双终端）**：

```bash
run_driver_qinglong.sh    # 终端1（PC）
run_sim2sim_qinglong.sh   # 终端2（K3 板卡）
```
...
| 现象 | 处理 |
| --- | --- |
| `[PolicyConfigLoader] ONNX 模型文件不存在` | 运行 `download_models_qinglong.sh` 下载模型后重试 |
| 进程启动后通信无数据 | 检查 `config/qinglong.yaml` 中 `driver_ip` / `control_ip` 是否正确，确认两机可互相 ping 通 |
...
| 版本 | 说明 |
| --- | --- |
| 0.1.0 | 初始版本，支持青龙 31-DOF FSM 与 sim2sim 仿真 |
